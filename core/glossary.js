...
export const TERMS = [
  ...
  /* ---------- 정렬 ---------- */
  {
    id: 'mark', term: '마크', en: 'Fiducial Mark', category: 'align',
    short: '정렬 기준으로 삼는, 위치를 아는 표식.',
    body: '십자·원·모서리 등을 씁니다. 비전이 찾아낸 실측 위치와 도면상 목표 위치의 차이가 곧 보정량입니다. 두 개면 회전까지, 세 개 이상이면 스케일과 직교도까지 나눠 볼 수 있습니다.',
    calc: 'mark-align', related: ['mark-distance', 'mark-detect-error', 'align-error'],
  },
  {
    id: 'mark-distance', term: '마크 간 거리', en: 'Mark Distance', category: 'align',
    short: '정렬 마크 두 개가 떨어져 있는 거리.',
    body: '각도 정밀도를 좌우하는 값입니다. 같은 검출 오차라도 거리가 2 배면 각도 오차는 절반이 됩니다. 마크를 대상 양 끝으로 최대한 벌려 두는 이유입니다.',
    calc: 'align-budget', related: ['mark', 'angular-error', 'align-error'],
  },
  {
    id: 'mark-detect-error', term: '마크 검출 오차', en: 'Mark Detection Error', category: 'align',
    short: '비전이 마크 중심을 찾을 때 남는 위치 오차.',
    body: '대상 분해능 × 서브픽셀 검출 오차입니다. 서브픽셀까지 쓰면 0.1~0.3 px 정도로 봅니다. 마크가 흐리거나 조명이 불균일하면 이 값이 커지고, 정렬 정밀도의 출발점이 그만큼 나빠집니다.',
    calc: 'align-budget', related: ['mark', 'edge-error', 'spatial-resolution'],
  },
  {
    id: 'stroke', term: '이동량', en: 'Stroke', category: 'align',
    short: '스테이지 축이 실제로 움직여야 하는 거리.',
    body: '정렬 계산이 내놓은 ΔX·ΔY·Δθ 를 기구 배치에 맞춰 각 축의 값으로 바꾼 것입니다. 축마다 스트로크 한계가 있으므로 이 값이 한계를 넘지 않는지 확인해야 합니다.',
    calc: 'stage-kinematics', related: ['uvw-stage', 'rotation-angle', 'rot-center'],
  },
  {
    id: 'rotation-angle', term: '회전각', en: 'Rotation Angle', category: 'align',
    aliases: ['θ축 회전량'],
    short: '대상을 목표 자세로 맞추기 위해 돌려야 하는 각.',
    body: '두 마크를 잇는 벡터의 각도 차이로 구합니다. 정렬에서는 µrad 단위를 다루므로 도 단위로는 소수점 넷째 자리까지 봐야 의미가 있습니다. 1 µrad 은 반경 100 mm 에서 0.1 µm 입니다.',
    calc: 'mark-align', related: ['rot-center', 'shift', 'angular-error'],
  },
  {
    id: 'camera-rotation', term: '카메라 회전각', en: 'Camera Rotation', category: 'align',
    short: '카메라 픽셀 축과 스테이지 축이 틀어진 각도.',
    body: '스테이지를 한 축으로만 움직였는데 이미지에서 대각으로 움직이면 그만큼 틀어진 것입니다. 좌표 변환에 넣지 않으면 정렬할 때마다 직교축으로 오차가 새어 나갑니다.',
    calc: 'hand-eye', related: ['cross-coupling', 'calibration', 'rotation-angle'],
  },
  {
    id: 'scale-error', term: '스케일 오차', en: 'Scale Error', category: 'align',
    short: '실측한 마크 간 거리가 목표보다 늘거나 줄어든 비율.',
    body: 'ppm 으로 봅니다. 회전이나 병진으로는 지워지지 않는 성분이라, 이 값이 크면 캘리브레이션이 틀렸거나 대상이 열로 늘어난 것입니다. 유리는 1 °C 에 약 8 ppm 늘어납니다.',
    calc: 'mark-align', related: ['calibration', 'align-error', 'thermal-drift'],
  },
  {
    id: 'residual', term: '잔차', en: 'Residual', category: 'align',
    short: '보정을 하고도 남는 오차.',
    body: '반복 정렬에서는 회차가 갈수록 줄지만 반복 정밀도 아래로는 내려가지 않습니다. 마크 정합에서는 스케일을 보정하지 않고 회전·병진만 썼을 때 각 마크에 남는 몫을 뜻합니다.',
    calc: 'align-convergence', related: ['repeatability', 'convergence-ratio', 'align-error'],
  },
  {
    id: 'align-radius', term: '반경', en: 'Radius', category: 'align',
    short: '회전 중심에서 관심 지점까지의 거리.',
    body: '각도 오차가 실제 위치 오차로 바뀌는 지렛대 길이입니다. 반경이 길수록 같은 각도 오차가 더 큰 편차를 만들므로, 검사 영역에서 회전 중심에 가장 먼 지점을 기준으로 잡습니다.',
    calc: 'rotation-center', related: ['rot-center', 'angular-error', 'edge-align-error'],
  },
  {
    id: 'averaging', term: '평균 횟수', en: 'Averaging', category: 'align',
    short: '같은 지점을 몇 번 찍어 평균 내는지.',
    body: '무작위 잡음은 √n 만큼 줄어듭니다. 4 회 평균이면 절반입니다. 다만 조명 기울기나 캘리브레이션 오차처럼 늘 같은 방향으로 생기는 계통 오차는 평균으로 줄지 않습니다.',
    calc: 'align-budget', related: ['mark-detect-error', 'repeatability'],
  },
  {
    id: 'translation-error', term: '병진 오차', en: 'Translation Error', category: 'align',
    short: '정렬 후 남는 위치 오차 중 회전과 무관한 몫.',
    body: '두 마크의 중점이 흔들리는 양입니다. 두 마크가 독립으로 흔들리므로 마크 하나의 검출 오차보다 √2 만큼 작습니다.',
    calc: 'align-budget', related: ['angular-error', 'align-error', 'mark-detect-error'],
  },
  {
    id: 'angular-error', term: '각도 오차', en: 'Angular Error', category: 'align',
    short: '정렬 후 남는 회전 오차.',
    body: '√2 × 마크 검출 오차 / 마크 간 거리입니다. 마크가 가까울수록 급격히 나빠집니다. µrad 단위로 다루며, 반경을 곱하면 그 지점에서의 실제 편차가 됩니다.',
    calc: 'align-budget', related: ['mark-distance', 'edge-align-error', 'rotation-angle'],
  },
  {
    id: 'edge-align-error', term: '끝단 오차', en: 'Edge Error', category: 'align',
    short: '각도 오차가 대상 끝에서 만드는 위치 오차.',
    body: '반경 × 각도 오차입니다. 대상이 클수록 커지므로, 중앙에서는 잘 맞는데 가장자리에서 틀어지는 현상의 원인입니다. 병진 오차보다 크면 마크 간 거리부터 늘려야 합니다.',
    calc: 'align-budget', related: ['angular-error', 'align-radius', 'align-error'],
  },
  {
    id: 'align-error', term: '정렬 오차', en: 'Alignment Error', category: 'align',
    short: '병진과 각도 성분을 합친 최종 정렬 오차.',
    body: '두 성분이 서로 무관하므로 제곱합으로 더합니다. 공차와 비교할 때는 보통 3σ 값을 씁니다. 오차 배분에서 정렬이 차지하는 몫이 바로 이 값입니다.',
    calc: 'align-budget', related: ['translation-error', 'edge-align-error', 'combined-error'],
  },
  {
    id: 'rot-center', term: '회전 중심', en: 'Rotation Center', category: 'align',
    aliases: ['회전 중심 X', '회전 중심 Y'],
    short: '스테이지가 실제로 그 둘레를 도는 점.',
    body: '기준점과 떨어져 있으면 θ 만 돌려도 기준점이 호를 따라 밀립니다. 이 밀림을 XY 로 상쇄하지 않으면 회전을 맞출수록 위치가 틀어져 정렬 루프가 수렴하지 않습니다.',
    calc: 'rotation-center', related: ['shift', 'align-radius', 'reference-point'],
  },
  {
    id: 'reference-point', term: '기준점', en: 'Reference Point', category: 'align',
    aliases: ['기준점 X', '기준점 Y'],
    short: '정렬을 맞추려는 대상 위의 한 점.',
    body: '보통 대상 중심이나 실제 검사·가공이 일어나는 위치로 잡습니다. 어디를 기준점으로 잡느냐에 따라 회전 반경이 달라지고, 회전 보정량도 달라집니다.',
    calc: 'rotation-center', related: ['rot-center', 'align-radius'],
  },
  {
    id: 'shift', term: '밀림', en: 'Shift', category: 'align',
    short: '회전 때문에 기준점이 옮겨 간 거리.',
    body: '2 × 회전 반경 × sin(θ/2) 입니다. 반경 150 mm 에서 0.02° 만 돌려도 52 µm 가 밀립니다. 정렬 공차가 µm 단위라면 절대 무시할 수 없는 크기입니다.',
    calc: 'rotation-center', related: ['rot-center', 'align-radius', 'stroke'],
  },
  {
    id: 'uvw-stage', term: 'UVW 스테이지', en: 'UVW Stage', category: 'align',
    aliases: ['U축 Y좌표', 'V축 Y좌표', 'W축 X좌표', 'U − V'],
    short: '액추에이터 세 개로 XYθ 를 동시에 내는 정렬 스테이지.',
    body: '두 축이 한 방향, 나머지 한 축이 직교 방향으로 구동합니다. 같은 방향 두 축의 차이가 회전을 만들고 평균이 병진을 만듭니다. 적층 구조가 없어 낮고 강성이 좋습니다.',
    calc: 'stage-kinematics', related: ['stroke', 'rotation-angle', 'rot-center'],
  },
  {
    id: 'initial-error', term: '초기 오차', en: 'Initial Error', category: 'align',
    short: '정렬을 시작하기 전의 어긋난 양.',
    body: '로더가 올려놓는 정밀도로 정해집니다. 이 값이 크면 그만큼 반복 횟수가 늘어 택트가 나빠지므로, 기구적인 예비 정렬을 두는 경우가 많습니다.',
    calc: 'align-convergence', related: ['convergence-ratio', 'iteration', 'residual'],
  },
  {
    id: 'loop-gain', term: '루프 게인', en: 'Loop Gain', category: 'align',
    short: '계산한 보정량의 몇 배를 실제로 내리는지.',
    body: '1 이면 계산값 그대로입니다. 1 보다 작으면 안전하지만 느리고, 1 을 넘으면 매번 목표를 지나쳐 진동하며, 2 이상이면 발산합니다. 실무에서는 0.7~0.9 를 많이 씁니다.',
    calc: 'align-convergence', related: ['convergence-ratio', 'iteration', 'residual'],
  },
  {
    id: 'repeatability', term: '반복 정밀도', en: 'Repeatability', category: 'align',
    short: '같은 동작을 반복했을 때 결과가 흩어지는 폭.',
    body: '정렬이 도달할 수 있는 바닥입니다. 아무리 반복해도 이 아래로는 내려가지 않으므로, 목표 공차가 이 값과 비슷하면 횟수를 늘려도 소용이 없습니다.',
    calc: 'align-convergence', related: ['residual', 'align-error', 'gage-repeat'],
  },
  {
    id: 'iteration', term: '반복 횟수', en: 'Iterations', category: 'align',
    aliases: ['최대 반복', '필요 반복 횟수'],
    short: '공차 안에 들어올 때까지 정렬을 돌리는 횟수.',
    body: '한 번에 몇 초씩 걸리므로 택트에 바로 영향을 줍니다. 초기 오차와 루프 게인으로 정해지며, 보통 2~3 회 안에 끝나도록 게인과 예비 정렬을 잡습니다.',
    calc: 'align-convergence', related: ['loop-gain', 'initial-error', 'convergence-ratio'],
  },
  {
    id: 'convergence-ratio', term: '수렴비', en: 'Convergence Ratio', category: 'align',
    short: '한 번 돌릴 때 오차가 몇 배로 줄어드는지.',
    body: '|1 − 루프 게인| 입니다. 0.2 면 매회 오차가 5 분의 1 로 줄어듭니다. 1 이상이면 줄지 않거나 커지므로 게인을 다시 잡아야 합니다.',
    calc: 'align-convergence', related: ['loop-gain', 'iteration', 'residual'],
  },
  {
    id: 'cross-coupling', term: '직교축 누설', en: 'Cross Coupling', category: 'align',
    short: '한 축만 움직였는데 반대 축으로 새어 나가는 양.',
    body: '카메라 회전각의 탄젠트입니다. 0.1° 틀어져 있으면 X 로 1 mm 갈 때 Y 로 1.7 µm 가 따라옵니다. 이동량이 클수록 비례해 커지므로 넓은 대상을 다룰수록 문제가 됩니다.',
    calc: 'hand-eye', related: ['camera-rotation', 'calibration'],
  },
  {
    id: 'travel-distance', term: '이동 거리', en: 'Travel Distance', category: 'align',
    aliases: ['스테이지 ΔX', '스테이지 ΔY', '픽셀 Δu', '픽셀 Δv'],
    short: '핸드아이 보정에서 일부러 움직여 본 양.',
    body: '스테이지에 내린 명령값과 그때 이미지에서 특징이 움직인 픽셀 수를 짝지어 봅니다. 크게 움직일수록 회전각과 분해능이 정확해지므로, 시야를 거의 가로지를 만큼 주는 것이 좋습니다.',
    calc: 'hand-eye', related: ['camera-rotation', 'calibration', 'spatial-resolution'],
  },

  /* ---------- 공정능력 ---------- */
  {
    id: 'usl', term: 'USL', en: 'Upper Specification Limit', category: 'capability',
    short: '규격 상한. 이보다 크면 불량.',
    body: '도면이나 사양서가 정하는 값이며 공정이 실제로 낼 수 있는 값과는 무관합니다. LSL 과의 간격이 공차 폭이고, 공정능력은 그 폭 안에 산포가 얼마나 여유 있게 들어오는지로 정해집니다.',
    calc: 'process-capability', related: ['lsl', 'tolerance-width', 'cp'],
  },
  {
    id: 'lsl', term: 'LSL', en: 'Lower Specification Limit', category: 'capability',
    short: '규격 하한. 이보다 작으면 불량.',
    body: '한쪽 규격만 있는 특성도 많습니다. 그럴 때는 Cpu 나 Cpl 중 존재하는 쪽만 보며, Cp 는 정의되지 않습니다.',
    calc: 'process-capability', related: ['usl', 'tolerance-width', 'cpu-cpl'],
  },
  {
    id: 'mean', term: '평균', en: 'Mean', category: 'capability',
    short: '측정값들의 중심 위치.',
    body: '규격 중앙에서 벗어난 만큼 Cpk 가 Cp 보다 떨어집니다. 편심은 대개 오프셋 보정으로 잡을 수 있어, 산포를 줄이는 것보다 손대기 쉬운 경우가 많습니다.',
    calc: 'process-capability', related: ['offset-ratio', 'cp', 'cpk'],
  },
  {
    id: 'std-dev', term: '표준편차', en: 'Standard Deviation', category: 'capability',
    short: '측정값이 평균에서 흩어진 정도.',
    body: '공정능력의 분모입니다. 관측한 표준편차에는 공정 산포와 측정계 산포가 함께 들어 있으므로, 측정계가 거칠면 실제보다 나쁘게 보입니다.',
    calc: 'process-capability', related: ['spread', 'gage', 'cp'],
  },
  {
    id: 'cp', term: 'Cp', en: 'Process Capability', category: 'capability',
    short: '공차 폭이 산포 폭(6σ)의 몇 배인지.',
    body: '평균이 어디 있든 상관하지 않고 산포만 봅니다. 그래서 Cp 는 좋은데 Cpk 가 나쁘면 문제는 산포가 아니라 편심입니다. 1.33 이면 공차가 산포의 1.33 배라는 뜻입니다.',
    calc: 'process-capability', related: ['cpk', 'mean', 'tolerance-width'],
  },
  {
    id: 'cpk', term: 'Cpk', en: 'Process Capability Index', category: 'capability',
    short: '평균의 치우침까지 반영한 공정능력.',
    body: 'Cpu 와 Cpl 중 나쁜 쪽입니다. 양산 기준은 보통 1.33(약 32 PPM), 안전 특성은 1.67 을 요구합니다. 1 미만이면 분포가 이미 규격을 물고 있습니다.',
    calc: 'process-capability', related: ['cp', 'cpu-cpl', 'sigma-level', 'ppm'],
  },
  {
    id: 'cpu-cpl', term: 'Cpu', en: 'Upper Capability', category: 'capability',
    aliases: ['Cpl'],
    short: '위쪽·아래쪽 규격까지 각각 몇 σ 가 남았는지를 3 으로 나눈 값.',
    body: 'Cpu 는 상한까지, Cpl 은 하한까지의 여유입니다. 둘의 차이가 클수록 평균이 한쪽으로 몰려 있다는 뜻이고, 작은 쪽이 Cpk 가 됩니다.',
    calc: 'process-capability', related: ['cpk', 'mean', 'usl', 'lsl'],
  },
  {
    id: 'cpm', term: 'Cpm', en: 'Taguchi Capability Index', category: 'capability',
    short: '목표값에서 벗어난 몫을 산포와 같이 취급한 지수.',
    body: '규격 안에 들어오기만 하면 되는 것이 아니라 목표에 가까울수록 좋다고 볼 때 씁니다. 편심에 Cpk 보다 민감해서, 중심을 맞추는 것이 중요한 특성에 적합합니다.',
    calc: 'process-capability', related: ['cpk', 'mean', 'offset-ratio'],
  },
  {
    id: 'ppm', term: 'PPM', en: 'Parts per Million', category: 'capability',
    short: '100 만 개 중 규격을 벗어나는 개수.',
    body: '정규분포를 가정하고 규격 밖 꼬리 면적으로 계산합니다. Cpk 1.0 이면 약 2700 PPM, 1.33 이면 약 32 PPM 입니다. 실제 분포는 꼬리가 두꺼운 경우가 많아 이 값은 낙관적인 하한으로 보는 편이 안전합니다.',
    calc: 'process-capability', related: ['cpk', 'yield-rate', 'sigma-level'],
  },
  {
    id: 'sigma-level', term: '시그마 수준', en: 'Sigma Level', category: 'capability',
    short: '평균에서 가장 가까운 규격까지 몇 σ 인지.',
    body: '3 × Cpk 입니다. 흔히 말하는 6 시그마는 Cpk 2.0 에 해당합니다. 장기 변동을 감안해 1.5σ 를 빼고 부르는 관행이 있어, 같은 숫자라도 정의를 확인해야 합니다.',
    calc: 'process-capability', related: ['cpk', 'ppm'],
  },
  {
    id: 'offset-ratio', term: '편심률', en: 'Offset Ratio', category: 'capability',
    aliases: ['중심 정렬 이득'],
    short: '평균이 규격 중앙에서 얼마나 벗어났는지의 비율.',
    body: '0 이면 정중앙, 1 이면 한쪽 규격선 위입니다. Cpk = Cp × (1 − 편심률) 관계가 성립하므로, 이 값만 줄여도 Cpk 가 Cp 까지 회복됩니다.',
    calc: 'process-capability', related: ['mean', 'cp', 'cpk'],
  },
  {
    id: 'yield-rate', term: '수율', en: 'Yield', category: 'capability',
    short: '규격 안에 들어오는 비율.',
    body: '100 % 에서 PPM 을 환산해 뺀 값입니다. Cpk 가 1.33 만 넘어가면 수율은 99.99 % 대에서 거의 변하지 않으므로, 개선 효과를 보려면 수율보다 PPM 이나 Cpk 로 보는 편이 낫습니다.',
    calc: 'process-capability', related: ['ppm', 'cpk'],
  },
  {
    id: 'tolerance-width', term: '공차 폭', en: 'Tolerance', category: 'capability',
    aliases: ['목표 공차'],
    short: 'USL 과 LSL 사이의 간격.',
    body: '설계가 허용한 전체 폭입니다. 정렬·측정·스테이지·열 같은 오차원이 모두 이 안에서 자리를 나눠 써야 하므로, 오차 배분의 총예산이 됩니다.',
    calc: 'tolerance-budget', related: ['usl', 'lsl', 'combined-error', 'pt-ratio'],
  },
  {
    id: 'spread', term: '산포', en: 'Variation', category: 'capability',
    short: '값이 흩어진 폭.',
    body: '보통 6σ 로 나타냅니다. 관측 산포는 실제 공정 산포와 측정계 산포가 제곱합으로 섞인 값이라, 측정계를 재서 빼내야 진짜 공정 산포가 나옵니다.',
    calc: 'gage-rr', related: ['std-dev', 'gage', 'combined-error'],
  },
  {
    id: 'gage', term: '측정계', en: 'Measurement System', category: 'capability',
    short: '측정에 관여하는 장비·절차·사람 전체.',
    body: '카메라와 렌즈만이 아니라 조명, 지그, 알고리즘, 조작 절차까지 포함합니다. 측정계 산포가 공차의 상당 부분을 잡아먹으면 합부 판정 자체를 믿을 수 없습니다.',
    calc: 'gage-rr', related: ['gage-repeat', 'gage-reprod', 'pct-grr', 'pt-ratio'],
  },
  {
    id: 'gage-repeat', term: '반복성', en: 'Repeatability', category: 'capability',
    short: '같은 사람이 같은 대상을 반복 측정했을 때의 산포.',
    body: '장비 자체의 잡음입니다. 조명 흔들림, 초점, 에지 검출 알고리즘이 주된 원인입니다. AIAG 에서는 EV(Equipment Variation)라고 부릅니다.',
    calc: 'gage-rr', related: ['gage-reprod', 'gage', 'repeatability'],
  },
  {
    id: 'gage-reprod', term: '재현성', en: 'Reproducibility', category: 'capability',
    short: '측정자나 장비를 바꿨을 때 생기는 차이.',
    body: 'AIAG 의 AV(Appraiser Variation)입니다. 이 값이 크면 사람마다 티칭이나 셋업 절차가 다르다는 뜻이므로, 장비가 아니라 작업 표준을 손봐야 합니다.',
    calc: 'gage-rr', related: ['gage-repeat', 'gage', 'pct-grr'],
  },
  {
    id: 'pct-grr', term: '%GRR', en: 'Percent Gage R&R', category: 'capability',
    short: '전체 산포 중 측정계가 차지하는 비율.',
    body: 'AIAG 기준으로 10 % 미만이면 양호, 10~30 % 는 조건부 사용, 30 % 초과면 사용 불가입니다. 부품 산포가 좁으면 같은 측정계라도 이 값이 나빠지므로, 시료를 실제 생산 범위에서 골라야 합니다.',
    calc: 'gage-rr', related: ['pt-ratio', 'ndc', 'gage'],
  },
  {
    id: 'pt-ratio', term: 'P/T 비', en: 'Precision to Tolerance', category: 'capability',
    short: '측정계 산포(6σ)가 공차 폭의 몇 %인지.',
    body: '%GRR 이 부품 산포와 비교한 값이라면, 이쪽은 공차와 비교한 값입니다. 합부 판정이 목적이라면 P/T 가 더 직접적인 지표입니다. 30 % 를 넘으면 판정 자체를 믿기 어렵습니다.',
    calc: 'gage-rr', related: ['pct-grr', 'tolerance-width', 'gage'],
  },
  {
    id: 'ndc', term: 'NDC', en: 'Number of Distinct Categories', category: 'capability',
    short: '측정계가 부품 산포를 몇 단계로 구분할 수 있는지.',
    body: '1.41 × 부품 산포 / 측정계 산포입니다. 5 이상을 요구합니다. 이보다 작으면 측정값이 사실상 몇 개 계단으로만 나뉘어, 공정 변화를 추적할 수 없습니다.',
    calc: 'gage-rr', related: ['pct-grr', 'gage', 'spread'],
  },
  {
    id: 'combined-error', term: '합성 오차', en: 'Combined Error', category: 'capability',
    short: '여러 오차원을 하나로 합친 값.',
    body: '서로 무관한 오차는 제곱합(RSS)으로 더합니다. 단순히 더한 값보다 훨씬 작게 나오는데, 모든 오차가 같은 방향으로 동시에 몰릴 확률이 낮기 때문입니다. 같은 방향으로 늘 함께 움직이는 계통 오차에는 쓰면 안 됩니다.',
    calc: 'tolerance-budget', related: ['contribution', 'tolerance-width', 'align-error'],
  },
  {
    id: 'meas-error', term: '측정 오차', en: 'Measurement Error', category: 'capability',
    short: '측정계가 만들어 내는 오차 몫.',
    body: 'Gage R&R 이 내놓는 측정계 산포를 그대로 씁니다. 공차 배분에서 이 몫이 크면 공정을 개선해도 결과가 좋아지지 않습니다.',
    calc: 'tolerance-budget', related: ['gage', 'combined-error', 'uncertainty'],
  },
  {
    id: 'stage-error', term: '스테이지 오차', en: 'Stage Error', category: 'capability',
    short: '스테이지가 지시한 위치에 서지 못하는 양.',
    body: '위치결정 반복 정밀도와 직진도가 섞인 값입니다. 정렬 계산이 아무리 정확해도 이 값보다 잘 설 수는 없으므로, 정렬 공차의 하한을 정합니다.',
    calc: 'tolerance-budget', related: ['repeatability', 'combined-error', 'stroke'],
  },
  {
    id: 'thermal-drift', term: '열 변형', en: 'Thermal Drift', category: 'capability',
    short: '온도가 변해 대상이나 구조물이 늘어나는 양.',
    body: '유리는 1 °C 에 약 8 ppm, 알루미늄은 23 ppm 늘어납니다. 500 mm 대상이 1 °C 오르면 유리라도 4 µm 가 늘어나므로, µm 급 정렬에서는 무시할 수 없습니다. 마크 정합의 스케일 오차로 나타납니다.',
    calc: 'tolerance-budget', related: ['scale-error', 'combined-error'],
  },
  {
    id: 'contribution', term: '기여율', en: 'Contribution', category: 'capability',
    short: '합성 오차에서 각 항목이 차지하는 몫.',
    body: '제곱 기준이라 큰 항목이 훨씬 크게 잡힙니다. 두 배 큰 항목은 네 배를 차지합니다. 그래서 가장 큰 하나를 줄이는 것이 나머지를 모두 줄이는 것보다 효과적인 경우가 많습니다.',
    calc: 'tolerance-budget', related: ['combined-error', 'tolerance-width'],
  },
  {
    id: 'tol-usage', term: '공차 사용률', en: 'Tolerance Usage', category: 'capability',
    short: '총 산포(6σ)가 공차 폭의 몇 %를 쓰는지.',
    body: '100 % 면 Cp 가 정확히 1 입니다. 통상 기준인 Cp 1.33 을 맞추려면 75 % 이하로 써야 하고, 나머지는 장기 변동과 편심을 위한 여유로 남깁니다.',
    calc: 'tolerance-budget', related: ['cp', 'tolerance-width', 'combined-error'],
  },
];
